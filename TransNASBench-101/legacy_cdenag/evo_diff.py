...
def evo_diff(
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"[EvoDiff-TransNAS] Starting evolution process")
        print(f"  - Task: {task}, Search space: {search_space}")
        ...
        for t, alpha in bar:
            ...
            if use_consistency_generator:
                generator = ConsistencyGenerator(
                    ...
                    arch_fitness_fn=arch_fitness,   # TransNAS 适应度函数
                    api=api,
                    task=task,
            ...
            if not is_single_step:
                if t != num_step - 1:
                    ...
                    x = select(task=task, search_space=search_space, x_prev=x_prev, x_next=x,
                               elite_rate=elite_rate, diver_rate=diver_rate, api=api, max_iter_time=max_iter_time)
                    x_prev = copy.deepcopy(x)
                else:
    ...
    if plot_results:
        plot_denoise(save_dir=save_dir, avg_acc_trace=avg_acc_trace, max_acc_trace=max_acc_trace,
                     seed=seed, dataset=task)
    end_time = time.time()

    if verbose:
        print(f"\n{'='*60}")
        print(f"[EvoDiff-TransNAS] Evolution completed")
    ...
    # 最终评估
    if task == "room_layout":
        accurancy, fitness = arch_fitness(operation_matrix=x, api=api, task=task, search_space=search_space)
        max_acc = min(accurancy.min().item(), max_acc_trace[-1])
    else:
        accurancy, fitness = arch_fitness(operation_matrix=x, api=api, task=task, search_space=search_space)
...
from utils.plot import plot_denoise
from utils.corrector import crossover, mutate, select
from utils.predictor import BayesianGenerator, ConsistencyGenerator
from utils.ddim import DDIMSchedulerCosine
from utils.corrector import TimeoutException
from utils.transnasbench101_fitness import arch_fitness
...
def evo_diff(
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"[EvoDiff] Starting evolution process (TransNAS-Bench-101)")
        print(f"  - Task: {task}")
    ...
    try:
        for t, alpha in bar:
            ...
            if use_consistency_generator:
                ...
                generator = BayesianGenerator(
                    x=x, fitness=fitness, alpha=alpha, density='uniform', h=0.1, elite_strategy=False, verbose=verbose and (t == 0)
                )
    ...
    if verbose:
        print(f"\n{'='*60}")
        print(f"[EvoDiff] Evolution completed (TransNAS-Bench-101)")
        print(f"  - Final max accuracy: {max_acc_trace[-1]:.4f}")
        print(f"  - Total time: {end_time - start_time:.2f}s")
