...
class ConsistencyGenerator:
    """
    通过 EMA 混合引入历史最佳架构的引导，包含稳定性感知锦标赛选择 (SATS)。
    适用于 NAS-Bench-101。
    """
    def __init__(self, x, fitness, alpha, density='uniform', h=0.1, elite_strategy=False,
        ...
        self.api = api
        self.fitness_kwargs = fitness_kwargs   # 存储额外参数
    ...
    def _compute_stability_single(self, x_single):
        ...
        for _ in range(self.stability_perturb_num):
            ...
            # NAS-Bench-101 的适应度函数返回 (accuracy, fitness) 或更多，根据具体实现调整索引
            _, fit = self.arch_fitness_fn(adj_matrix=x_pert, nb_api=self.api)
...
# ====================== 完整的 ConsistencyGenerator（NAS-Bench-101 适配版） ======================
class ConsistencyGenerator:
    """
    完整的 Consistency Generator，直接包含所有实现，无需外部依赖
    适配 NAS-Bench-101 的搜索空间
    """
    def __init__(self, x, fitness, alpha, density='uniform', h=0.1, elite_strategy=False,
                 teacher_model=None, lambda_kl=0.5, lambda_ce=1.0, lambda_consistency=1.0,
                 consistency_type='mse', perturb_scale=0.1, arch_fitness_fn=None, api=None,
        ...
        if self.verbose:
            print(f"\n{'='*60}")
            print(f"[Generator Info] Using ConsistencyGenerator (NAS-Bench-101)")
            print(f"  - Tournament size: {self.tournament_size}")
