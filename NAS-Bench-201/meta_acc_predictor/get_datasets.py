...
MODULE_DATA_ROOT = Path(__file__).resolve().parent / "data"
PROJECT_LEGACY_DATA_ROOT = Path(__file__).resolve().parents[2] / "meta_acc_predictor" / "data"


def _resolve_data_root(root):
    """Return a cwd-independent dataset root.

    The original CDENAG helper used ``meta_acc_predictor/data`` relative to
    the process working directory.  That only works after ``cd
    NAS-Bench-201`` and fails for the documented project-root command
    ``python NAS-Bench-201/main.py``.  The default is now anchored to this
    module while an explicit absolute/custom root remains supported.
    """
    if root is None:
        return str(MODULE_DATA_ROOT)
    value = str(root).replace("\\", "/").rstrip("/")
    if value in ("meta_acc_predictor/data", "./meta_acc_predictor/data"):
        return str(MODULE_DATA_ROOT)
...
def _dataset_is_ready(name, root):
    """Check only complete prepared datasets, never temporary archives."""
    root = Path(root)
    if name == "aircraft":
        layouts = (
            root / "fgvc-aircraft-2013b" / "data",
            root / "aircraft" / "fgvc-aircraft-2013b" / "data",
        )
        return any(
            (layout / "images_manufacturer_trainval.txt").is_file()
            and (layout / "images_manufacturer_test.txt").is_file()
            and (layout / "images").is_dir()
            for layout in layouts
        )
    if name == "pets":
        return (
            (root / "pets" / "train85.pth").is_file()
            and (root / "pets" / "test15.pth").is_file()
        )
    return False


def _resolve_dataset_data_root(name, root=None):
    """Reuse module-local or old project-root datasets without duplication."""
    raw = "" if root is None else str(root).replace("\\", "/").rstrip("/")
    uses_default = root is None or raw in (
        "meta_acc_predictor/data",
        "./meta_acc_predictor/data",
    )
    primary = Path(_resolve_data_root(root)).resolve()
    if not uses_default or name not in ("aircraft", "pets"):
        return str(primary)

    candidates = (
        primary,
        PROJECT_LEGACY_DATA_ROOT.resolve(),
        (Path.cwd() / "meta_acc_predictor" / "data").resolve(),
    )
    seen = set()
    for candidate in candidates:
        ...
        if key in seen:
            continue
        seen.add(key)
        if _dataset_is_ready(name, candidate):
...
class FGVCAircraft(Dataset):
  ...
  def __init__(self, root, class_type='variant', split='train', transform=None,
    ...
    direct_root = os.path.join(data_root, 'fgvc-aircraft-2013b')
    legacy_root = os.path.join(data_root, 'aircraft', 'fgvc-aircraft-2013b')
    candidates = (direct_root, legacy_root)
    self.root = direct_root
    for candidate in candidates:
      marker = os.path.join(
          candidate, 'data',
          'images_%s_%s.txt' % (class_type, split),
      )
...
def get_datasets(name, cutout, root=None):
    root = _resolve_dataset_data_root(name, root)
    if name == 'cifar10':
    ...
    if name == 'cifar10':
        ...
        if not (os.path.isfile(direct_marker) or os.path.isfile(legacy_marker)):
            download_aircraft(root)
        ...
        if not os.path.isfile(os.path.join(root, 'pets', 'train85.pth')):
            download_pets(root)
        if not os.path.isfile(os.path.join(root, 'pets', 'test15.pth')):
            download_pets(root)
